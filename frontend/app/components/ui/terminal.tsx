  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Served from frontend/public/sound/sound.ogg
        const res = await fetch("/sound/sound.ogg");
        if (!res.ok) return;
        bufferRef.current = await ctx.decodeAudioData(await res.arrayBuffer());
        readyRef.current = true;
      } catch {
        /* optional asset — silent if missing */
      }
    };
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-xl border border-neutral-800 bg-neutral-900 shadow-2xl">
        ...
        <div
          ref={contentRef}
          className="h-72 overflow-y-auto p-4 font-mono sm:h-80"
        >
          {lines.map((line, i) => (
